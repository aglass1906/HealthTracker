-- Expose only aggregate health summaries to members of the same community.
-- Base nutrition rows remain owner-only under their existing RLS policies.

CREATE OR REPLACE FUNCTION public.community_member_summary(
  community_id uuid,
  member_id uuid
)
RETURNS TABLE (

-- Include active macro targets so community summaries can show aggregate
-- nutrition progress without exposing individual meal details.
...
RETURNS TABLE (
  ...
  protein_goal_g double precision,
  carb_goal_g double precision,
  fat_goal_g double precision,
...
AS $$
  ...
  active_nutrition_goal AS (
    SELECT goals.protein_g, goals.carb_g, goals.fat_g
    FROM public.nutrition_goals goals, authorized
    WHERE authorized.allowed
      AND goals.user_id = member_id
      AND goals.is_active
    ORDER BY goals.updated_at DESC
    LIMIT 1
  ),
  ...
  SELECT
    ...
    active_nutrition_goal.protein_g,
    active_nutrition_goal.carb_g,
    active_nutrition_goal.fat_g,
    latest_update.updated_at,
    latest_update.type
  FROM authorized
  LEFT JOIN latest_stat ON true
  LEFT JOIN nutrition_summary ON true
  LEFT JOIN active_nutrition_goal ON true
